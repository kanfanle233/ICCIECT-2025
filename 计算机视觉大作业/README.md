...
所有 **8 个视频**球检测质量均为 **Green**。本轮结果来自 `output/ball_quality_summary.csv/json`，使用已并入主流程默认值的 aggressive `--refine-ball` 参数，并已同步导出到 `frontend/public/data/manifest.json`：

| 视频 | 质量评分 | 可见率 | 最大缺失间隔 | 等级 |
|------|---------|--------|-------------|------|
| pro_match17_1_15_13 | 98.77 | 100% | 0 帧 | 🟢 Green |
| 1_00_01 | 98.67 | 100% | 0 帧 | 🟢 Green |
| pro_match19_1_01_01 | 98.51 | 81% | 40 帧 | 🟢 Green |
| pro_match17_2_18_11 | 98.29 | 95% | 23 帧 | 🟢 Green |
| pro_match17_2_15_11 | 98.21 | 96% | 20 帧 | 🟢 Green |
| pro_match17_2_08_05 | 98.02 | 100% | 0 帧 | 🟢 Green |
| pro_match17_2_01_01 | 97.92 | 90% | 36 帧 | 🟢 Green |
| pro_match17_1_02_02 | 97.90 | 87% | 30 帧 | 🟢 Green |
...
当前真实交付物包括：
- `*_overlay.mp4`：叠加球员分析、羽毛球高亮圆圈与短轨迹的分析视频
- `*_final.mp4`：最终渲染版本，默认保留羽毛球高亮跟踪效果
...
新增基于 D3.js 的交互式可视化仪表盘，支持 **8 个已分析视频** 的多维数据探索。
...
```bash
# 推荐：用本地 HTTP 服务打开，保证模块加载和 JSON/CSV 请求正常
...
```bash
python3 scripts/tools/select_court.py inputs/pro_match19_1_01_01.mp4
```

弹窗里**按顺序**点 4 下：左上 → 右上 → 右下 → 左下（球场长方形 4 角，不是球网）。点完按 q，终端会输出像 `--court_points "418,424,878,424,941,710,228,708"` 这样的字符串。

样本视频 `pro_match19_1_01_01.mp4` 的标准答案：

```
418,424,878,424,941,710,228,708
```
...
python3 run_pipeline.py \
  ...
  --court-points "418,424,878,424,941,710,228,708"
```

这会依次执行 TrackNet → Overlay → FX → 前端数据导出，输出在 `output/pro_match19_1_01_01/` 目录下：

```
output/pro_match19_1_01_01/
├── pro_match19_1_01_01_ball.csv          # 球的逐帧坐标
├── pro_match19_1_01_01_players.csv       # 球员检测数据
├── pro_match19_1_01_01_motion.csv        # 运动统计
├── pro_match19_1_01_01_overlay.mp4       # 叠加分析视频
├── pro_match19_1_01_01_final.mp4         # 最终成品视频
└── ...
```
...
```bash
# 播放叠加分析视频
open output/pro_match19_1_01_01/pro_match19_1_01_01_overlay.mp4
...
# 默认会在 overlay/final 视频中高亮羽毛球；如需关闭
python3 run_pipeline.py \
  --input-video inputs/pro_match19_1_01_01.mp4 \
  --court-points "418,424,878,424,941,710,228,708" \
  --no-draw-ball-highlight

# 调整高亮样式（BGR 颜色、圆圈半径、轨迹长度）
python3 run_pipeline.py \
  --input-video inputs/pro_match19_1_01_01.mp4 \
  --court-points "418,424,878,424,941,710,228,708" \
  --ball-highlight-color "0,235,60" \
  --ball-highlight-radius 10 \
  --ball-trail-len 10
...
| `--tracknet-eval-mode` | `nonoverlap` / `weight` | `nonoverlap` 快 8 倍 |
...
- 最大插值空洞扩展到 6 帧，默认 aggressive 参数为 `--ball-refine-static-run-frames 15`、`--ball-refine-static-radius-px 2.5`、`--ball-refine-max-dir-change-deg 175`、`--ball-refine-sigma-q 10`、`--ball-refine-sigma-r 3`、`--ball-refine-bottom-pad-px 50`
...
**Q：前端仪表盘打不开 / 没有数据**
A：确认 `frontend/public/data/manifest.json` 存在，并用 `cd frontend && python3 -m http.server 8080` 启动本地服务后访问 `http://localhost:8080`。运行 `run_pipeline.py` 会自动导出前端数据，或手动运行 `python3 scripts/tools/export_frontend_data.py`。
...
│   ├── pro_match19_1_01_01/        # 默认示例视频分析结果
