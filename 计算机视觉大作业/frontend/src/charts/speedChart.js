export function createSpeedChart(container, onSeek, bus) {
  ...
  function updateData(data, nextMetric = metric) {
    ...
    if (metric === "confidence") {
      if (series.ball.length) {
        g.append("path")
          ...
          .attr("d", area)
          .attr("fill", "rgba(232, 148, 90, 0.08)")
          .style("pointer-events", "none");
      }
      g.append("path")
        .datum(series.near)
        ...
        .attr("d", area)
        .attr("fill", "rgba(199, 91, 122, 0.08)")
        .style("pointer-events", "none");
      g.append("path")
        .datum(series.far)
        .attr("class", "series-area far-area")
        .attr("d", area)
        .attr("fill", "rgba(91, 168, 160, 0.08)")
        .style("pointer-events", "none");
    }
    ...
    svg.on("click", (event) => {
      ...
      const frame = Math.max(0, Math.min(Math.round(x.invert(mx)), data.analysis.frame_count - 1));
      onSeek(frame);
