export function createVisibilityTimeline(container, onSeek, bus) {
  const width = 520;
  const height = 120;
  ...
  const highlight = g.append("rect")
    ...
    .attr("width", 3).attr("rx", 1)
    .attr("fill", "var(--blue)").attr("opacity", 0)
  ...
  if (bus) {
    bus.on("select", (d) => {
      highlight.attr("x", x(d.frame) - 1.5).attr("opacity", 0.35);
    });
    bus.on("clearSelect", () => {
  ...
  function frameFromPointer(event) {
    ...
    return Math.max(0, Math.min(Math.round(x.invert(mx)), state.analysis.frame_count - 1));
  }
  ...
  function moveScrub(event) {
    if (isDragging) scrubToPointer(event);
  }
  ...
  function updateData(data) {
    ...
    g.selectAll(".tick-rect")
      ...
      .on("click", (event, d) => {
        if (!isDragging) {
          onSeek(d.frame, { pause: true });
          if (bus) bus.emit("select", { frame: d.frame });
        }
      });
    ...
    svg
      ...
      .on("pointerup pointercancel", endScrub)
      .on("mousedown", startScrub)
      .on("mousemove", moveScrub)
      .on("mouseup mouseleave", endScrub)
      .on("click", (event) => {
        if (event.target.tagName !== "rect") {
          const frame = frameFromPointer(event);
          onSeek(frame, { pause: true });
          if (bus) bus.emit("select", { frame });
        }
      })
      .on("dblclick", () => {
        if (bus) bus.emit("clearSelect");
      });
