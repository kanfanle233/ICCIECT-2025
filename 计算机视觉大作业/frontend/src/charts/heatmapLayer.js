  ...
  function updateData(data, nextMode = mode) {
    ...
    if (mode === "separated") {
      ...
      for (const r of data.ball.filter((d) => !d.is_missing && d.court_x_m !== null && d.court_y_m !== null)) {
        drawPoint(scaleX_A(r.court_x_m), scaleY_A(r.court_y_m), "rgba(232, 148, 90, 0.22)", splitRadius);
      }
      ...
      for (const r of nearRows) {
        drawPoint(scaleX_B(r.court_x_m), scaleY_B(r.court_y_m), "rgba(199, 91, 122, 0.2)", splitRadius);
      }
      ...
      for (const r of farRows) {
        drawPoint(scaleX_C(r.court_x_m), scaleY_C(r.court_y_m), "rgba(91, 168, 160, 0.2)", splitRadius);
      }
    } 
    else {
      ...
      if (mode === "all" || mode === "ball") {
        for (const row of data.ball.filter((d) => !d.is_missing && d.court_x_m !== null && d.court_y_m !== null)) {
          drawPoint(scaleX(row.court_x_m), scaleY(row.court_y_m), "rgba(232, 148, 90, 0.18)", radius);
        }
      }
      if (mode === "all" || mode === "near" || mode === "far") {
        ...
        for (const row of rows) {
          drawPoint(scaleX(row.court_x_m), scaleY(row.court_y_m), row.role === "near" ? "rgba(199, 91, 122, 0.16)" : "rgba(91, 168, 160, 0.16)", radius);
        }
      }
    }
  }
