...
export async function loadAllVideoDatasets(manifest) {
  const results = await Promise.allSettled(
    manifest.videos.map(async (entry) => [entry.id, await loadVideoDataset(entry)]),
  );
  const datasets = new Map();
  const errors = [];
  for (const result of results) {
    if (result.status === "fulfilled") {
      const [videoId, dataset] = result.value;
      datasets.set(videoId, dataset);
    } else {
