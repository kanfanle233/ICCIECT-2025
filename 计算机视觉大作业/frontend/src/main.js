import { loadAllVideoDatasets, loadManifest, loadVideoDataset } from "./dataLoader.js?v=diagnostics-20260604";
...
import { createCourtHeatGrid } from "./charts/courtHeatGrid.js?v=diagnostics-20260604";
import { renderQualityPanel, renderStatCards } from "./charts/statCards.js?v=spatial-merge-20260527";
import { renderFrameDetail } from "./charts/frameDetail.js?v=diagnostics-20260604";
import {
  createAllVideoTimeline,
  createQualityMatrix,
  createSourceStack,
  createSpatialValidity,
  createWorkloadPanel,
} from "./charts/diagnosticsCharts.js?v=diagnostics-20260604";
...
const els = {
  ...
  frameDetailPanel: document.querySelector("#frameDetailPanel"),
  ...
  spatialValidity: document.querySelector("#spatialValidity"),
  sourceStack: document.querySelector("#sourceStack"),
  allVideoTimeline: document.querySelector("#allVideoTimeline"),
  workloadPanel: document.querySelector("#workloadPanel"),
...
const charts = {
  miniCourt: createMiniCourt(document.querySelector("#miniCourt"), seekFrame, bus),
  speed: createSpeedChart(els.speedChart, seekFrame, bus),
  timeline: createVisibilityTimeline(document.querySelector("#visibilityTimeline"), seekFrame, bus),
  heatGrid: createCourtHeatGrid(document.querySelector("#courtHeatGrid"), els.zoneInsight, seekFrame, bus),
  qualityMatrix: createQualityMatrix(document.querySelector("#qualityMatrix"), selectVideoFrame),
  sourceStack: createSourceStack(els.sourceStack, selectVideoFrame),
  allVideoTimeline: createAllVideoTimeline(els.allVideoTimeline, selectVideoFrame),
  spatialValidity: createSpatialValidity(els.spatialValidity),
  workload: createWorkloadPanel(els.workloadPanel),
};
...
function seekFrame(frame, options = {}) {
  sync.seekFrame(Math.max(0, Math.min(frame, (state.dataset?.analysis?.frame_count || 1) - 1)), options);
}
...
function renderFrame(frame) {
  ...
  charts.heatGrid.renderFrame(frame);
  charts.spatialValidity.renderFrame(frame);
  charts.workload.renderFrame(frame);
  renderFrameDetail(els.frameDetailPanel, state.dataset, frame);
  if (current?.ball) {
...
function updateTemporalView() {
  const targetByMetric = {
    speed: els.speedChart,
    distance: els.speedChart,
    confidence: els.speedChart,
    spatial: els.spatialValidity,
    sources: els.sourceStack,
    "all-videos": els.allVideoTimeline,
    workload: els.workloadPanel,
  };
  const activeTarget = targetByMetric[state.metric] || els.speedChart;
  for (const target of [els.speedChart, els.spatialValidity, els.sourceStack, els.allVideoTimeline, els.workloadPanel]) {
    if (target) target.hidden = target !== activeTarget;
  }
}

function renderAllDatasetDiagnostics() {
  if (!state.allDatasets || state.allDatasets.size === 0) return;
  const selectedId = state.dataset?.entry?.id;
  charts.qualityMatrix.updateData(state.allDatasets, selectedId);
  charts.sourceStack.updateData(state.allDatasets, selectedId);
  charts.allVideoTimeline.updateData(state.allDatasets, selectedId);
}
...
function updateTemporalLegend() {
  if (state.metric === "spatial") {
    els.temporalLegend.innerHTML = [
      ["detected", "Green: valid court projection"],
      ["image-only", "Blue: image-only / invalid projection"],
      ["ball", "Orange: interpolated"],
      ["low", "Yellow: low confidence"],
      ["missing", "Red: missing"],
    ]
      .map(([colorClass, text]) => `<span><i class="swatch ${colorClass}"></i>${text}</span>`)
      .join("");
    return;
  }
  if (state.metric === "sources") {
    els.temporalLegend.innerHTML = [
      ["detected", "Green: refined"],
      ["ball", "Orange: interpolated"],
      ["missing", "Red: missing"],
      ["image-only", "Blue: ROI rejected"],
      ["near", "Pink: jump rejected"],
    ]
      .map(([colorClass, text]) => `<span><i class="swatch ${colorClass}"></i>${text}</span>`)
      .join("");
    return;
  }
  if (state.metric === "all-videos") {
    els.temporalLegend.innerHTML = [
      ["detected", "Green: valid"],
      ["image-only", "Blue: image-only"],
      ["ball", "Orange: interpolated"],
      ["low", "Yellow: low confidence"],
      ["missing", "Red: missing"],
    ]
  ...
  if (state.metric === "workload") {
    els.temporalLegend.innerHTML = [
      ["near", "Pink: near-player rolling speed"],
      ["far", "Cyan: far-player rolling speed"],
      ["image-only", "Blue: distance differential"],
    ]
      .map(([colorClass, text]) => `<span><i class="dot ${colorClass}"></i>${text}</span>`)
      .join("");
    return;
  }
...
function renderDataset(data, targetFrame = 0, options = {}) {
  state.dataset = data;
  ...
  charts.speed.updateData(data, ["speed", "distance", "confidence"].includes(state.metric) ? state.metric : "speed");
  charts.timeline.updateData(data);
  charts.heatGrid.updateData(data);
  charts.spatialValidity.updateData(data);
  charts.workload.updateData(data);
  updateTemporalView();
  updateTemporalLegend();
  renderAllDatasetDiagnostics();
  const nextSource = data.urls[state.source] ? state.source : "overlay";
  setVideoSource(nextSource);
  seekFrame(targetFrame, { pause: Boolean(options.pause) });
}

async function loadSelectedVideo(videoId, frame = 0, options = {}) {
  const entry = state.manifest.videos.find((item) => item.id === videoId);
  if (!entry) return;
  let dataset = state.allDatasets.get(videoId);
  if (!dataset) {
    dataset = await loadVideoDataset(entry);
    state.allDatasets.set(videoId, dataset);
  }
  els.videoSelect.value = videoId;
  renderDataset(dataset, frame, options);
}

async function selectVideoFrame(videoId, frame = 0) {
  await loadSelectedVideo(videoId, frame, { pause: true });
}
...
function setupControls() {
  ...
  els.videoSelect.addEventListener("change", () => loadSelectedVideo(els.videoSelect.value, 0, { pause: true }));
  ...
  for (const button of document.querySelectorAll("[data-metric]")) {
    button.addEventListener("click", () => {
      ...
      updateTemporalView();
      if (["speed", "distance", "confidence"].includes(state.metric)) {
  ...
  for (const button of document.querySelectorAll("[data-heat-mode]")) {
    button.addEventListener("click", () => {
      state.heatMode = button.dataset.heatMode;
      document.querySelectorAll("[data-heat-mode]").forEach((item) => item.classList.toggle("active", item === button));
      charts.heatGrid.setMode(state.heatMode);
      renderFrame(state.frame);
    });
  }
...
async function boot() {
  ...
  const loaded = await loadAllVideoDatasets(state.manifest);
  state.allDatasets = loaded.datasets;
  state.diagnosticsErrors = loaded.errors;
  setupControls();
  await loadSelectedVideo(state.manifest.default_video, 0, { pause: true });
}
