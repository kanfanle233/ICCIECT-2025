...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ball-refine-static-run-frames", type=int, default=15,
                         help="Static-lock: minimum consecutive near-identical frames to reject.")
    parser.add_argument("--ball-refine-static-radius-px", type=float, default=2.5,
                         help="Static-lock: radius in pixels to consider a point stationary.")
    parser.add_argument("--ball-refine-max-dir-change-deg", type=float, default=175.0,
                         help="Max direction change in degrees for jump rejection.")
    parser.add_argument("--ball-refine-sigma-q", type=float, default=10.0,
                         help="Kalman process noise std for smoothing.")
    parser.add_argument("--ball-refine-sigma-r", type=float, default=3.0,
                         help="Kalman measurement noise std for smoothing.")
    parser.add_argument("--ball-refine-bottom-pad-px", type=float, default=50.0,
                         help="Bottom ROI padding in pixels for --refine-ball.")
    parser.add_argument(
        "--tracknet-eval-mode",
    ...
    parser.add_argument("--draw-ball-highlight", action="store_true", default=True,
                        help="Draw ball highlight circle and trail on the overlay video (default: on).")
    parser.add_argument("--no-draw-ball-highlight", action="store_false", dest="draw_ball_highlight",
                        help="Disable ball highlight drawing.")
    parser.add_argument("--ball-highlight-color", default="0,235,60",
                        help="Ball highlight outer circle BGR color as comma-separated ints.")
    parser.add_argument("--ball-highlight-radius", type=int, default=10,
                        help="Ball highlight outer circle radius in pixels.")
    parser.add_argument("--ball-trail-len", type=int, default=10,
                        help="Number of recent ball positions to draw as trail.")
    parser.add_argument(
        "--no-frontend-export",
        action="store_true",
...
def main() -> int:
    ...
    if args.refine_ball:
        ...
        refine_cmd = [
            ...
            "--static-run-frames",
            str(args.ball_refine_static_run_frames),
            "--static-radius-px",
            str(args.ball_refine_static_radius_px),
            "--max-dir-change-deg",
            str(args.ball_refine_max_dir_change_deg),
            "--sigma-q",
            str(args.ball_refine_sigma_q),
            "--sigma-r",
            str(args.ball_refine_sigma_r),
            "--bottom-pad-px",
            str(args.ball_refine_bottom_pad_px),
            "--report-json",
    ...
    if args.draw_ball_highlight:
        overlay_cmd.append("--draw_ball_highlight")
        overlay_cmd += ["--ball_highlight_color", args.ball_highlight_color]
        overlay_cmd += ["--ball_highlight_radius", str(args.ball_highlight_radius)]
        overlay_cmd += ["--ball_trail_len", str(args.ball_trail_len)]
    else:
        overlay_cmd.append("--no_draw_ball_highlight")
