...
def resolve_device(requested):
    """Select CUDA on NVIDIA hosts, MPS on Apple Silicon, then CPU."""
...
def open_video_writer(path, fps, size):
    """Prefer browser-friendly AVC, but keep desktop generation portable."""
    ...
    for codec in ("avc1", "H264", "mp4v"):
        ...
        if writer.isOpened():
            if codec != "avc1":
                print(f"[WARN] Video writer using {codec}; browser preview may require H.264 transcoding.")
            return writer, codec
        writer.release()
...
def assign_players(
    ...
    if has_side_line:
        # New tracks belong to one side only. Existing IDs keep their role:
        # players do not cross the net, while perspective can place a true
        # near-side foot above an image-space divider.
        near_pool = [c for c in candidates_sorted if c["foot"][1] >= side_line_y]
        far_pool = [c for c in candidates_sorted if c["foot"][1] < side_line_y]
        if prev_near_id is not None:
            near_pool.extend(
                c
                for c in candidates_sorted
                if c["track_id"] == prev_near_id
                and c not in near_pool
            )
        if prev_far_id is not None:
            far_pool.extend(
                c
                for c in candidates_sorted
                if c["track_id"] == prev_far_id
                and c not in far_pool
            )
...
class MotionStats:
    ...
    def to_dict(self):
        return {
            ...
            "rally_distance_m": float(self.rally_distance),
            "rally_time_s": float(self.rally_time),
            "rally_max_speed_mps": float(self.rally_max_speed),
            "rally_avg_speed_mps": float(self.rally_avg_speed),
            "total_distance_m": float(self.total_distance),
            "total_time_s": float(self.total_time),
            "total_max_speed_mps": float(self.total_max_speed),
            "total_avg_speed_mps": float(self.total_avg_speed),
        }
...
def draw_ball_highlight(frame, x, y, outer_color, radius):
    """Draw a glowing ball highlight: outer ring + yellow center dot."""
    cv2.circle(frame, (x, y), radius, outer_color, 2, cv2.LINE_AA)
    cv2.circle(frame, (x, y), radius + 3, outer_color, 1, cv2.LINE_AA)
    cv2.circle(frame, (x, y), max(2, radius // 3), (0, 235, 255), -1, cv2.LINE_AA)


def draw_ball_trail(frame, trail, color, thickness=2):
    """Draw a polyline trail from recent ball positions (oldest→newest)."""
    ...
    for i in range(1, len(pts)):
        ...
        cv2.line(frame, pts[i - 1], pts[i], seg_color, thickness, cv2.LINE_AA)
...
def run(args):
    ...
    writer, output_codec = open_video_writer(args.output_path, fps, (w, h))

    model = YOLO(args.yolo_model)
    resolved_device = resolve_device(args.device)
    use_half = bool(args.half and resolved_device.startswith("cuda"))
    ...
    ball_highlight_trail = deque(maxlen=max(1, args.ball_trail_len))

    if args.draw_ball_on_minimap or args.draw_ball_highlight:
    ...
    print(f"[INFO] device={resolved_device}, half={use_half}, output_codec={output_codec}")
    ...
    while True:
        ...
        # --- Ball highlight on main frame ---
        if args.draw_ball_highlight and frame_idx in ball_dict:
            vis, bx, by = ball_dict[frame_idx]
            if vis > 0:
                ball_highlight_trail.append((int(bx), int(by)))
                draw_ball_trail(frame, ball_highlight_trail, args.ball_highlight_color)
                draw_ball_highlight(frame, int(bx), int(by), args.ball_highlight_color, args.ball_highlight_radius)
            else:
                ball_highlight_trail.append(None)
        elif args.draw_ball_highlight:
            ball_highlight_trail.append(None)
        ...
        near_box = near_player_draw.get("box") if near_player_draw is not None else None
        far_box = far_player_draw.get("box") if far_player_draw is not None else None
        near_source = "detected" if near_player is not None else ("tracked" if near_draw_pt is not None else "missing")
        far_source = "detected" if far_player is not None else ("tracked" if far_draw_pt is not None else "missing")

        # Collect frame trajectory & motion data for both the renderer and frontend export.
        frame_records.append({
            "Frame": frame_idx,
            "Time_s": frame_idx / max(fps, 1e-6),
            "Near_X": near_draw_pt[0] if near_draw_pt is not None else 0,
            "Near_Y": near_draw_pt[1] if near_draw_pt is not None else 0,
            "Far_X": far_draw_pt[0] if far_draw_pt is not None else 0,
            "Far_Y": far_draw_pt[1] if far_draw_pt is not None else 0,
            "Near_Court_X_m": near_m_for_map[0] if near_m_for_map is not None else "",
            "Near_Court_Y_m": near_m_for_map[1] if near_m_for_map is not None else "",
            "Far_Court_X_m": far_m_for_map[0] if far_m_for_map is not None else "",
            "Far_Court_Y_m": far_m_for_map[1] if far_m_for_map is not None else "",
            "Near_Track_ID": near_player_draw.get("track_id", "") if near_player_draw is not None else "",
            "Far_Track_ID": far_player_draw.get("track_id", "") if far_player_draw is not None else "",
            "Near_Confidence": near_player_draw.get("confidence", 0.0) if near_player_draw is not None else 0.0,
            "Far_Confidence": far_player_draw.get("confidence", 0.0) if far_player_draw is not None else 0.0,
            "Near_BBox": near_box,
            "Far_BBox": far_box,
            "Near_Source": near_source,
            "Far_Source": far_source,
            "Near_Speed_mps": near_stats.current_speed,
            "Near_Dist_m": near_stats.total_distance,
            "Near_Rally_Dist_m": near_stats.rally_distance,
            "Near_Max_Speed_mps": near_stats.total_max_speed,
            "Far_Speed_mps": far_stats.current_speed,
            "Far_Dist_m": far_stats.total_distance,
            "Far_Rally_Dist_m": far_stats.rally_distance,
            "Far_Max_Speed_mps": far_stats.total_max_speed,
        })
    ...
    if args.stats_json:
        video_id = os.path.splitext(os.path.basename(args.output_path))[0].replace("_overlay", "")
        stats_ball_csv = ball_csv or args.ball_csv
        stats_payload = {
            "video_id": video_id,
            "video_path": args.video_path,
            "output_path": args.output_path,
            "ball_csv": stats_ball_csv,
            ...
            "fps": float(fps),
            "duration_s": float(frame_idx / max(fps, 1e-6)),
            "rally_count": int(rally_idx),
            "detect_calls": int(detect_calls),
            "detect_ratio": float(detect_ratio),
            "court_points": court_quad.astype(float).tolist(),
            "court_points_px": court_quad.astype(float).tolist(),
            "court_size_m": {
                "width": float(args.court_width_m),
                "length": float(args.court_length_m),
            },
            "homography": homography.astype(float).tolist(),
            "ball_center_shift_m": float(ball_x_shift),
            "players": {
                "far": far_stats.to_dict(),
                "near": near_stats.to_dict(),
            },
        }
        ...
        print(f"[DONE] stats json: {args.stats_json}")

    if args.players_csv:
        os.makedirs(os.path.dirname(os.path.abspath(args.players_csv)), exist_ok=True)
        with open(args.players_csv, "w", encoding="utf-8", newline="") as f:
            csv_w = csv.writer(f)
            csv_w.writerow([
                "frame",
                "time_s",
            ...
            for r in frame_records:
                for role, prefix in (("near", "Near"), ("far", "Far")):
                    box = r.get(f"{prefix}_BBox") or ("", "", "", "")
                    x_px = r.get(f"{prefix}_X", 0)
                    y_px = r.get(f"{prefix}_Y", 0)
                    court_x = r.get(f"{prefix}_Court_X_m", "")
                    court_y = r.get(f"{prefix}_Court_Y_m", "")
                    confidence = r.get(f"{prefix}_Confidence", 0.0)
                    csv_w.writerow([
                        r["Frame"],
                        f"{r['Time_s']:.6f}",
                        role,
                        r.get(f"{prefix}_Track_ID", ""),
                        f"{x_px:.3f}" if x_px else "",
                        f"{y_px:.3f}" if y_px else "",
                        f"{court_x:.6f}" if court_x != "" else "",
                        f"{court_y:.6f}" if court_y != "" else "",
                        f"{box[0]:.3f}" if box[0] != "" else "",
                        f"{box[1]:.3f}" if box[1] != "" else "",
                        f"{box[2]:.3f}" if box[2] != "" else "",
                        f"{box[3]:.3f}" if box[3] != "" else "",
                        f"{confidence:.6f}",
                        r.get(f"{prefix}_Source", "missing"),
                    ])
        print(f"[DONE] players csv: {args.players_csv}")

    if args.motion_csv:
        os.makedirs(os.path.dirname(os.path.abspath(args.motion_csv)), exist_ok=True)
        with open(args.motion_csv, "w", encoding="utf-8", newline="") as f:
            csv_w = csv.writer(f)
            csv_w.writerow([
                "frame",
                "time_s",
            ...
            for r in frame_records:
                for role, prefix in (("near", "Near"), ("far", "Far")):
                    speed = r.get(f"{prefix}_Speed_mps", 0.0)
                    distance = r.get(f"{prefix}_Dist_m", 0.0)
                    rally_distance = r.get(f"{prefix}_Rally_Dist_m", 0.0)
                    max_speed = r.get(f"{prefix}_Max_Speed_mps", 0.0)
                    csv_w.writerow([
                        r["Frame"],
                        f"{r['Time_s']:.6f}",
                        role,
        ...
        print(f"[DONE] motion csv: {args.motion_csv}")
...
def build_arg_parser():
    ...
    p.add_argument("--players_csv", type=str, default="")
    p.add_argument("--motion_csv", type=str, default="")
    ...
    p.add_argument("--half", dest="half", action="store_true", help="Enable FP16 on CUDA; ignored on MPS/CPU for stability")
    ...
    p.add_argument("--draw_ball_highlight", dest="draw_ball_highlight", action="store_true")
    p.add_argument("--no_draw_ball_highlight", dest="draw_ball_highlight", action="store_false")
    p.set_defaults(draw_ball_highlight=False)
    p.add_argument("--ball_highlight_color", type=lambda s: tuple(int(x) for x in s.split(",")),
                   default=(0, 235, 60), help="Outer circle BGR color for ball highlight.")
    p.add_argument("--ball_highlight_radius", type=int, default=10, help="Outer circle radius in pixels.")
    p.add_argument("--ball_trail_len", type=int, default=10, help="Number of recent ball positions to draw as trail.")

    p.add_argument("--draw_court_polygon", action="store_true")
    p.add_argument("--draw_embedded_panels", dest="draw_embedded_panels", action="store_true")
    p.add_argument("--no_draw_embedded_panels", dest="draw_embedded_panels", action="store_false")
    p.set_defaults(draw_embedded_panels=True)
