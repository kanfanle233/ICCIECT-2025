"""
基于 HMM 的中文分词实现（两分类：YN）。

教学重点：与 BMES 四分类不同，两分类方案将每个字标注为：
Y = 词的最后一个字（即此处应切分），N = 非最后一个字。
模型更简单，适合理解 HMM 分词的基本原理。
"""
...
def train(corpus_path):
    ...
    """
    pi_dict = {s: 0.0 for s in 'YN'}
    A_dict = {s: {s_prime: 0.0 for s_prime in 'YN'} for s in 'YN'}
    B_dict = {s: {} for s in 'YN'}
    state_count = {s: 0.0 for s in 'YN'}
    line_count = 0
        ...
        for line in f:
            ...
            for word in words:
                ...
                if len(word) == 1:
                    ...
                    state_list.extend(['N'] * (len(word) - 1) + ['Y'])
    ...
    # 归一化为概率
    pi_prob = {s: (pi_dict[s] / line_count) if line_count > 0 else 0 for s in 'YN'}
    A_prob = {s: {s_prime: (A_dict[s][s_prime] / state_count[s]) if state_count[s] > 0 else 0 
                  for s_prime in 'YN'} for s in 'YN'}
    B_prob = {s: {c: count / state_count[s] for c, count in B_dict[s].items()} for s in 'YN'}
    
    # 写入JSON文件
    with open('pi_2.json', 'w', encoding='utf-8') as f:
        json.dump(pi_prob, f, ensure_ascii=False, indent=4)
    with open('A_2.json', 'w', encoding='utf-8') as f:
        json.dump(A_prob, f, ensure_ascii=False, indent=4)
    with open('B_2.json', 'w', encoding='utf-8') as f:
...
def load_model():
    """读取训练好的两分类JSON文件模型"""
    with open('pi_2.json', 'r', encoding='utf-8') as f:
        pi_prob = json.load(f)
    with open('A_2.json', 'r', encoding='utf-8') as f:
        A_prob = json.load(f)
    with open('B_2.json', 'r', encoding='utf-8') as f:
        B_prob = json.load(f)
    return pi_prob, A_prob, B_prob
...
def cut(text, corpus_path):
    ...
    """
    if not os.path.exists('pi_2.json') or not os.path.exists('A_2.json') or not os.path.exists('B_2.json'):
